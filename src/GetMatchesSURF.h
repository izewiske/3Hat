...
Plane matchStrengthsSimpleBounds(globalOri, cv::Mat mimg1, cv::Mat mimg2, cv::Mat bools1=NULL, cv::Mat bools2=NULL){
  ...
  // Convert/initialize boolean contour maps if applicable
  // Ignore features outside the contour, but process the pixels 
  IplImage *b1, *b2;
  if (bools1!=NULL && bools2!=NULL){
...
/* cimg1 and cimg2 are slices of images within a bounding box
   ...
   there will be no partial features.  */
Plane matchStrengthsContour(globalOri, cv::Mat cimg1, cv::Mat cimg2, cv::Mat bools1, cv::Mat bools2, partialFeatureAdjust=true){
  // Copy the images to avoid screwing up other stuff
